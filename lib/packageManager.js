...
const { execSync } = require('child_process'); // Ajoutez cette ligne
const fs = require('fs');
...
async function getLatestPackageInfo(packageName, distro) {
    console.log(`Recherche de l'URL du dernier package pour ${packageName} sur ${distro}...`);
    try {
        ...
        console.log(`Réponse reçue pour la recherche du package : ${response.status}`);
        ...
        console.log(`Package trouvé : ${pkg.name} (version ${pkg.version})`);
        const packageFilesResponse = await axios.get(`${GITLAB_API_URL}/${PROJECT_ID}/packages/${pkg.id}/package_files`);
        console.log(`Réponse reçue pour la recherche des fichiers du package : ${packageFilesResponse.status}`);
