...
async function getLatestBinariesPackageInfo(arch) {
    try {
        ...
        const packageFilesResponse = await axios.get(`${GITLAB_API_URL}/api/v4/projects/${binariesPackage.id}/package_files`);
        ...
        const packageFiles = {
            ...
            baker: packageFilesResponse.data.find(file => file.file_name.includes(`${arch}-octez-baker`))
        };
        ...
        return {
            ...
            bakerUrl: `${GITLAB_API_URL}/tezos/tezos/-/package_files/${packageFiles.baker.id}/download`
        };
    } catch (error) {
...
async function installTezosBaker(protocolHash) {
    ...
    if (installedBakerVersion === binariesPackageInfo.version) {
        console.log('octez-baker is already up to date. No action needed.');
        return;
    } else {
        console.log(`Current baker version: ${installedBakerVersion}`);
        console.log(`Latest version: ${binariesPackageInfo.version}`);
    }
