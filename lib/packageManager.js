...
async function getLatestBinariesPackageInfo(distro, version) {
    ...
    try {
        ...
        const possibleMatches = [`${normalizedDistro}-${version}`, `${distro}-${version}`, `${normalizedDistro}`, `${distro}`];

        // Filter to find the appropriate package for the detected distro and version
