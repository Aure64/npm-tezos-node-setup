...
async function getLatestBinariesPackageInfo(distro) {
    console.log(`[packageManager] Fetching latest binaries package info for distro: ${distro}`);
    try {
        ...
        response.data.forEach(pkg => console.log(`[packageManager] Package found: ${pkg.name}`));

        // Look for the correct package for the distro
        const binariesPackage = response.data.find(pkg =>
            pkg.name.includes(`octez-ubuntu-${distro}-20.2`) &&
        ...
        console.log(`[packageManager] Retrieved ${packageFilesResponse.data.length} files for package ${binariesPackage.name}`);
...
async function installTezosNode() {
    ...
    console.log(`[packageManager] Detected OS: ${distro}`);
