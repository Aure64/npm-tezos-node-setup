...
const ZCASH_PARAMS_URL = 'https://raw.githubusercontent.com/zcash/zcash/713fc761dd9cf4c9087c37b078bdeab98697bad2/zcutil/fetch-params.sh';
const ZCASH_PARAMS_DIRS = [
    '/_opam/share/zcash-params',
    '/usr/local/share/zcash-params',
    '/usr/share/zcash-params'
];

// Function to check if Zcash params are already installed
function areZcashParamsInstalled() {
    return ZCASH_PARAMS_DIRS.some(dir => fs.existsSync(dir) && fs.readdirSync(dir).length > 0);
}

// Function to download and install Zcash parameters
async function installZcashParams() {
    if (areZcashParamsInstalled()) {
        console.log('Zcash parameters already installed.');
        return;
    }

    console.log('Zcash parameters not found. Downloading and installing...');

    const tmpScriptPath = '/tmp/fetch-params.sh';

    try {
        // Download the fetch-params.sh script
        await downloadFile(ZCASH_PARAMS_URL, tmpScriptPath);

        // Make the script executable
        execSync(`chmod +x ${tmpScriptPath}`);

        // Run the script to install Zcash parameters
        execSync(`sudo ${tmpScriptPath}`);
