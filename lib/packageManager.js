...
async function installTezosBaker(protocolHash) {
    ...
    try {
        ...
        // If no binaries are found, continue to download the baker binary
    ...
    // Filter only binaries that include the protocol prefix and are baker binaries
    const matchedBakerBinary = bakerBinaries.find(binary => binary.includes(protocolPrefix) && binary.includes('baker'));
