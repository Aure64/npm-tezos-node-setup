...
async function getLatestBinariesPackageInfo(arch) {
    try {
        ...
        // Sélectionner uniquement les fichiers pertinents pour octez-node, octez-client et octez-baker
        const packageFiles = {
            ...
            baker: packageFilesResponse.data.find(file => file.file_name.includes(`${arch}-octez-baker-PsParisC`))
        };

        // Vérifier si les fichiers ont été trouvés
        if (!packageFiles.node || !packageFiles.client || !packageFiles.baker) {
            console.error(`Required package files not found for architecture ${arch}`);
            return null;
        }

        console.log(`Selected files: node, client, and baker for architecture ${arch}`);

        return {
            nodeUrl: `${GITLAB_API_URL}/${PROJECT_ID}/package_files/${packageFiles.node.id}/download`,
            clientUrl: `${GITLAB_API_URL}/${PROJECT_ID}/package_files/${packageFiles.client.id}/download`,
            bakerUrl: `${GITLAB_API_URL}/${PROJECT_ID}/package_files/${packageFiles.baker.id}/download`
        };
    } catch (error) {
