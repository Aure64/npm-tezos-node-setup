...
async function getLatestBinariesPackageInfo(arch, downloadBaker = false) {
    try {
        ...
        // Sélectionner uniquement les fichiers pertinents pour octez-node et octez-client, et baker si demandé
        const packageFiles = {
        ...
        console.log(`Selected files: node and client${downloadBaker ? ', and baker' : ''} for architecture ${arch}`);
...
async function installTezosNode() {
    ...
    if (isPackageInstalled('octez-client') && isPackageInstalled('octez-node')) {
        console.log('octez-client and octez-node are already installed.');
        return;
    }
...
async function installTezosBaker(protocolHash) {
    ...
    if (isPackageInstalled(`octez-baker-${protocolHash}`)) {
        console.log(`octez-baker-${protocolHash} is already installed.`);
        return;
    }
