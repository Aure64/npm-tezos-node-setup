...
async function installTezosBaker(protocolHash) {
    ...
    const installedBakerVersion = getInstalledVersion(`octez-baker-${protocolHash}`);
    ...
    if (installedBakerVersion === binariesPackageInfo.version) {
        console.log(`octez-baker-${protocolHash} is already up to date. No action needed.`);
        return;
    } else {
        console.log(`Current version of octez-baker-${protocolHash}: ${installedBakerVersion}`);
        console.log(`Latest version: ${binariesPackageInfo.version}`);
    }
    ...
    try {
        console.log(`Downloading octez-baker-${protocolHash}...`);
        const bakerFile = binariesPackageInfo.files.find(file => file.file_name.includes(`${arch}-octez-baker-${protocolHash}`));

        if (!bakerFile) {
            console.error(`No baker file found for protocol ${protocolHash} and architecture ${arch}`);
            return;
        }

        const bakerUrl = `${GITLAB_API_URL}/tezos/tezos/-/package_files/${bakerFile.id}/download`;
        await downloadFile(bakerUrl, tmpBakerPath);
        await installPackageOrBinary(tmpBakerPath, `octez-baker-${protocolHash}`);

        console.log(`octez-baker-${protocolHash} installed/updated successfully.`);
    } finally {
