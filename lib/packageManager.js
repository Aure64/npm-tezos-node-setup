...
async function getLatestPackageInfo(packageName, distro, arch) {
    try {
        ...
        const packageFile = packageFilesResponse.data.find(file => file.file_name.includes(`${arch}-${packageName}`));
        if (!packageFile) {
            console.error(`No package file found for ${packageName} for architecture ${arch}`);
            return null;
        }
...
async function installTezosNode() {
    ...
    const clientPackageInfo = await getLatestPackageInfo('octez-client', distro, arch);
    const nodePackageInfo = await getLatestPackageInfo('octez-node', distro, arch);

    if (!clientPackageInfo || !nodePackageInfo) {
        console.error('Unable to find package URLs for the specified distribution and architecture.');
        process.exit(1);
    }
...
async function installTezosBaker(protocolHash) {
    ...
    const bakerPackageInfo = await getLatestPackageInfo(`octez-baker-${protocolHash}`, distro, arch);
    ...
    const installedBaker = isPackageInstalled(`octez-baker-${protocolHash}`);
