...
async function configureBakerService(dataDir, rpcPort, serviceName, passwordFilePath = null) {
    ...
    // Ajouter l'option -f <path_to_password_file> si un fichier de mot de passe est fourni
    const passwordOption = passwordFilePath ? `-f ${passwordFilePath}` : '';
...
[Service]
ExecStart=/usr/bin/${bakerBinary} run with local node ${dataDir} --liquidity-baking-toggle-vote pass ${passwordOption}
Restart=on-failure
