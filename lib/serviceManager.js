...
async function configureBakerService(dataDir, rpcPort, serviceName) {
    const protocol = await getCurrentProtocol(rpcPort);
    const bakerBinary = `octez-baker-${protocol}`;
...
[Service]
ExecStart=/usr/bin/${bakerBinary} run with local node ${dataDir} --liquidity-baking-toggle-vote pass
Restart=on-failure
