...
// Fonction pour configurer le service Tezos Baker
async function configureBakerService(dataDir, rpcPort, serviceName, alias, passwordFilePath = null) {
    ...
    let execStartCmd = `/usr/bin/${bakerBinary} run with local node ${dataDir} ${alias} --liquidity-baking-toggle-vote pass`;

    // Ajout de l'option -f si le fichier de mot de passe est fourni
    if (passwordFilePath) {
        execStartCmd = `/usr/bin/${bakerBinary} -f ${passwordFilePath} run with local node ${dataDir} ${alias} --liquidity-baking-toggle-vote pass`;
    }
