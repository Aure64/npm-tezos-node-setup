...
WantedBy=multi-user.target
    ...
    sudo.exec(`echo "${serviceUnit}" | tee ${servicePath}`, options, (error, stdout, stderr) => {
        if (error) {
            console.error(`Erreur lors de la création du fichier service: ${error}`);
            return;
        }
        sudo.exec(`systemctl daemon-reload && systemctl enable ${serviceName} && systemctl start ${serviceName}`, options, (error, stdout, stderr) => {
            if (error) {
