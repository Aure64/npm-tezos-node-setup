...
// Function to sanitize service names by removing accents and preserving hyphens and underscores
function sanitizeServiceName(name) {
    return name
        .normalize('NFD') // Normalize the string to separate accents from characters
        .replace(/[\u0300-\u036f]/g, '') // Remove the accents
        .replace(/[^a-zA-Z0-9_-]/g, '_'); // Replace all non-alphanumeric characters (except hyphens and underscores) with an underscore
}
...
async function configureServiceUnit(dataDir, rpcPort, netPort, serviceName) {
    ...
    const escapedServiceName = execSync(`systemd-escape --suffix=service "${sanitizedServiceName}"`).toString().trim();
...
`;
    ...
    const servicePath = `/etc/systemd/system/${escapedServiceName}`;
    ...
    try {
        console.log(`Enabling and starting service: ${escapedServiceName}`);
        execSync(`sudo systemctl enable "${escapedServiceName}"`);
        execSync(`sudo systemctl start "${escapedServiceName}"`);
        await new Promise(resolve => setTimeout(resolve, 5000)); // Wait for 5 seconds
        execSync(`sudo systemctl restart "${escapedServiceName}"`);
        await new Promise(resolve => setTimeout(resolve, 3000)); // Wait for 3 seconds

        console.log(`Service ${escapedServiceName} started successfully.`);
    } catch (error) {
...
async function configureBakerService(dataDir, rpcPort, alias, passwordFilePath = null) {
    ...
    const escapedAlias = execSync(`systemd-escape --suffix=service "${sanitizedAlias}"`).toString().trim();
...
`;
    ...
    const servicePath = `/etc/systemd/system/${escapedAlias}`;
    ...
    // Enable and start the systemd service
    try {
        console.log(`Enabling and starting service: ${escapedAlias}`);
        execSync(`sudo systemctl enable "${escapedAlias}"`);
        execSync(`sudo systemctl start "${escapedAlias}"`);
        await new Promise(resolve => setTimeout(resolve, 5000)); // Wait for 5 seconds
        execSync(`sudo systemctl restart "${escapedAlias}"`);
        await new Promise(resolve => setTimeout(resolve, 3000)); // Wait for 3 seconds
        console.log(`Service ${escapedAlias} started successfully.`);
    } catch (error) {
        console.error(`Error while enabling/starting service: ${error.message}`);
        throw error;
    }
