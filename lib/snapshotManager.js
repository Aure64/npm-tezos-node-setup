...
function removeDirectoryWithSudo(directoryPath) {
    console.log(`Removing directory: ${directoryPath} with sudo...`);
    try {
        const command = `rm -rf ${directoryPath}`;
        sudo.exec(command, { name: 'Tezos Setup' }, (error, stdout, stderr) => {
            if (error) {
                console.error(`Failed to remove directory ${directoryPath}: ${stderr}`);
                throw error;
            } else {
...
async function importSnapshot(network, mode, dataDir, fastMode = false, snapshotPath) {
    ...
    try {
        ...
        processes.forEach(pid => {
            try {
                sudo.exec(`kill ${pid}`, { name: 'Tezos Setup' }, (error, stdout, stderr) => {
                    if (error) {
                        console.error(`Error stopping process ${pid}: ${stderr}`);
                    } else {
