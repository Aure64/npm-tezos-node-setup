...
async function importSnapshot(network, mode, dataDir, fastMode = false, snapshotPath, netPort) {
    ...
    try {
        // Check if netstat or ss command is available
        const checkNetstat = execSync('command -v netstat || command -v ss').toString().trim();
        if (!checkNetstat) {
            console.error('Neither netstat nor ss is available. Please install one of them and rerun the script.');
            process.exit(1);
        }
    } catch (error) {
        console.error('Failed to check for netstat/ss availability:', error.message);
        process.exit(1);
    }

    try {
        // Find and stop processes using the specified port
        const command = `sudo netstat -tuln | grep :${netPort} || sudo ss -tuln | grep :${netPort}`;
