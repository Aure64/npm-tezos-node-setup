...
async function setupBaker(dataDir, rpcPort, network) {
    ...
    if (currentBalance >= 6000) {
        console.log(`The balance for ${alias} is already ${formatBalance(currentBalance)} ꜩ, which is sufficient.`);
        const { continueSetup } = await inquirer.prompt([
            {
                type: 'confirm',
                name: 'continueSetup',
                message: 'Do you want to continue with the current balance?',
                default: true
            }
        ]);
            ...
            const { amount } = await inquirer.prompt([
                {
                    type: 'input',
                    name: 'amount',
                    message: `Enter the amount of ꜩ to request from the faucet (minimum ${amountNeeded.toFixed(6)} ꜩ to reach 6000 ꜩ):`,
