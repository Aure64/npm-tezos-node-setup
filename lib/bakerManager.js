...
async function setupBaker(dataDir, rpcPort, network) {
    ...
    if (setupService) {
        const isEncrypted = selectedAddress.includes('encrypted sk');
        if (isEncrypted) {
            ...
            const passwordFilePath = path.join(dataDir, `${alias}_password.txt`);
            fs.writeFileSync(passwordFilePath, password);

            await configureBakerService(dataDir, rpcPort, alias, passwordFilePath);

            // Supprimer le fichier de mot de passe après démarrage du service
            fs.unlinkSync(passwordFilePath);
            console.log('Password file has been deleted after starting the service.');
        } else {
