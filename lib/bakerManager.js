...
const { configureServiceUnit, configureBakerService } = require('./serviceManager');
...
function checkNodeBootstrapped() {
    ...
    while (true) {
        ...
        if (output.includes('Node is bootstrapped')) {
            ...
            setTimeout(() => { }, 10000); // Wait 10 seconds avant de réessayer
        }
    }
}
...
async function setupBaker(dataDir, rpcPort, network) {
    ...
    const { fundOption } = await inquirer.prompt([
        {
            ...
            choices: ['Self-fund', 'Use faucet', 'Continue']
        }
    ]);
    ...
    if (setupService) {
        await setupBakerService(dataDir, rpcPort, alias);
    }
...
async function setupBakerService(dataDir, rpcPort, alias) {
    const isEncrypted = alias.startsWith('encrypted');
    let passwordFilePath = null;

    if (isEncrypted) {
        // Demander le mot de passe pour la clé privée
        const { password } = await inquirer.prompt([
            {
                type: 'password',
                name: 'password',
                message: 'Enter the password for your encrypted private key:',
            }
        ]);

        // Créer un fichier temporaire pour stocker le mot de passe
        passwordFilePath = path.join(dataDir, `${alias}_password`);
        fs.writeFileSync(passwordFilePath, password, { mode: 0o600 });
    }

    try {
        // Configurer et démarrer le service baker
        await configureBakerService(dataDir, rpcPort, alias, passwordFilePath);
    } finally {
        // Supprimer le fichier contenant le mot de passe une fois le service démarré
        if (passwordFilePath && fs.existsSync(passwordFilePath)) {
            fs.unlinkSync(passwordFilePath);
            console.log(`Temporary password file ${passwordFilePath} deleted.`);
        }
    }
}
...
async function importSecretKey() {
    const { alias, secretKey } = await inquirer.prompt([
        ...
        },
        {
            type: 'password',
            name: 'secretKey',
            message: 'Enter the secret key (including "encrypted:" or "unencrypted:"):'
        }
    ]);
    ...
    try {
        ...
        // Utiliser directement la clé fournie par l'utilisateur
        execSync(`octez-client import secret key ${alias} ${secretKey}`, { stdio: 'inherit' });

        console.log(`Key ${alias} has been successfully imported.`);
    } catch (error) {
        console.error(`Failed to import secret key: ${error.message}`);
        throw error;
    }
