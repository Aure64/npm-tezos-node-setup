...
async function setupBaker(dataDir, rpcPort, network) {
    ...
    if (setupService) {
        if (isKeyEncrypted(alias)) {
            ...
            console.log(`Password file contents: ${fs.readFileSync(passwordFilePath, 'utf-8')}`);

            await configureBakerService(dataDir, rpcPort, alias, passwordFilePath);

            console.log(`Verifying password file at: ${passwordFilePath}`);
            if (fs.existsSync(passwordFilePath)) {
                console.log(`Password file exists. Contents: ${fs.readFileSync(passwordFilePath, 'utf-8')}`);
            } else {
                console.error(`Password file does not exist.`);
            }
