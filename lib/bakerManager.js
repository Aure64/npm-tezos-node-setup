...
let selectedBakerAddress = null;

// Function to set the selected baker address
function setAddress(address) {
    selectedBakerAddress = address;
}

// Function to get the selected baker address
function getAddress() {
    return selectedBakerAddress;
}
...
async function setupBaker(dataDir, rpcPort, network) {
    ...
    if (knownAddresses.length > 0) {
        ...
        if (keyOption === 'useExisting') {
            ...
            setAddress(showAddress(alias));  // Set the selected baker address
