...
function updateClientConfig(rpcPort) {
    ...
    // Forcer l'update dans le fichier de configuration
    execSync(`sed -i 's|"endpoint":.*|"endpoint": "${endpoint}",|' ~/.tezos-client/config`);
}
...
function formatBalance(balance) {
    return balance.toFixed(0); // Format balance as an integer (without decimals)
}
