...
function isKeyEncrypted(alias, knownAddresses) {
    // Find the address information from the list
    const addressInfo = knownAddresses.find(info => info.startsWith(`${alias}:`));
    return addressInfo && addressInfo.includes('(encrypted sk known)');
}
...
async function setupBaker(dataDir, rpcPort, network) {
    ...
    if (setupService) {
        ...
        if (keyIsEncrypted) {
            ...
            console.log(`Password file created successfully at: ${passwordFilePath}`);
            console.log(`Password file content: ${fs.readFileSync(passwordFilePath, 'utf8')}`);

            await configureBakerService(dataDir, rpcPort, alias, passwordFilePath);

            // Ajout d'un délai de 5 secondes avant la suppression du fichier
            console.log('Waiting 5 seconds before deleting the password file...');
            await new Promise(resolve => setTimeout(resolve, 5000));

            console.log(`Deleting password file: ${passwordFilePath}`);
            fs.unlinkSync(passwordFilePath);
            console.log(`Password file ${passwordFilePath} deleted.`);
        } else {
