...
function formatBalance(balance) {
    return balance.toString(); // Use default JavaScript string conversion
}
...
async function setupBaker(rpcPort, network) {
    ...
    if (neededAmount === 0) {
        console.log(`The address ${alias} already has sufficient funds: ${formatBalance(currentBalance)} ꜩ.`);
    } else {
        const { fundOption } = await inquirer.prompt([
            {
                type: 'list',
                name: 'fundOption',
                message: `The current balance is ${formatBalance(currentBalance)} ꜩ. How would you like to fund the baker key?`,
        ...
        if (fundOption === 'Self-fund') {
            ...
            console.log(`Please send at least ${formatBalance(neededAmount)} ꜩ to the address ${address}.`);
            ...
            const { amount } = await inquirer.prompt([
                {
                    type: 'input',
                    name: 'amount',
                    message: `Enter the amount of ꜩ to request from the faucet (minimum ${formatBalance(neededAmount)} ꜩ to reach 6000 ꜩ):`,
                    default: neededAmount,
                    validate: value => value >= neededAmount || `You must request at least ${formatBalance(neededAmount)} ꜩ.`
                }
            ]);
