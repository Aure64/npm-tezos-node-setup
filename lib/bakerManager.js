...
// Variable globale pour stocker l'adresse tz
let tzAddress = '';
...
function showAddress(alias) {
    ...
    console.log(`Retrieved address for alias ${alias}: ${address}`);  // Log the address retrieval
    return address;
}
...
function getBalance(alias) {
    try {
        ...
        console.log(`Retrieved balance for alias ${alias}: ${balance} ꜩ`);  // Log the balance retrieval
        return balance;
    } catch (error) {
...
function getTzAddress() {
    return tzAddress;
}

function setTzAddress(address) {
    tzAddress = address;
}
...
async function setupBaker(dataDir, rpcPort, network) {
    ...
    if (knownAddresses.length > 0) {
        ...
        if (keyOption === 'useExisting') {
            ...
            setTzAddress(showAddress(alias));  // Set tzAddress here
    ...
    console.log(`Final chosen tzAddress: ${tzAddress}`);  // Log the final chosen address
        ...
        const { fundOption } = await inquirer.prompt([
            {
                ...
                choices: [
                    'Self-fund',
                    'Use faucet',
                    'Register and Setup with Partial Funds'  // New option
                ]
            }
        ]);
    ...
    console.log(`Final chosen tzAddress: ${tzAddress} `);  // Log the final chosen address at the end of setup
}
