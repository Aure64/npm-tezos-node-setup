...
function listKnownAddressesWithBalances() {
    ...
    const addressesWithBalances = addresses.map(address => {
        ...
        return `${address} (Balance: ${balance.toFixed(6)} ꜩ)`;
    });
...
function getBalance(alias) {
    try {
        ...
        console.log(`Raw balance output for ${alias}: ${output}`); // Debugging line
        // Filtrer les lignes qui ne contiennent pas l'avertissement et obtenir la ligne avec le solde
...
async function setupBaker(rpcPort, network) {
    ...
    if (neededAmount === 0) {
        console.log(`The address ${alias} already has sufficient funds: ${currentBalance.toFixed(6)} ꜩ.`);
    } else {
        const { fundOption } = await inquirer.prompt([
            {
                type: 'list',
                name: 'fundOption',
                message: `The current balance is ${currentBalance.toFixed(6)} ꜩ. How would you like to fund the baker key?`,
        ...
        if (fundOption === 'Self-fund') {
            ...
            console.log(`Please send at least ${neededAmount.toFixed(6)} ꜩ to the address ${address}.`);
            ...
            const { amount } = await inquirer.prompt([
                {
                    type: 'input',
                    name: 'amount',
                    message: `Enter the amount of ꜩ to request from the faucet (minimum ${neededAmount.toFixed(6)} ꜩ to reach 6000 ꜩ):`,
                    default: neededAmount,
                    validate: value => value >= neededAmount || `You must request at least ${neededAmount.toFixed(6)} ꜩ.`
                }
            ]);
...
async function waitForBalance(alias, minBalance) {
    ...
    while (true) {
        ...
        if (balance >= minBalance) {
            console.log(`Balance of ${alias} is sufficient: ${balance.toFixed(6)} ꜩ.`);
            break;
        } else {
            console.log(`Current balance: ${balance.toFixed(6)} ꜩ. Waiting for funds...`);
