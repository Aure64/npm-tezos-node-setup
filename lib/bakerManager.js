...
async function setupBaker(rpcPort, network) {
    // Step 1: Update the octez-client configuration
    updateClientConfig(rpcPort);

    // Step 2: Check if the node is bootstrapped
    checkNodeBootstrapped();

    // Step 3: List known addresses and ask if the user wants to use an existing one
