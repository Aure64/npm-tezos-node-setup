        ...
        const { fundOption } = await inquirer.prompt([
            {
                type: 'list',
                name: 'fundOption',
                message: `The balance of ${alias} is ${currentBalance} ꜩ. How would you like to fund the baker key?`,
...
async function registerAsDelegate(alias, network) {
    ...
    const result = execSync(`octez-client register key ${alias} as delegate`, { stdio: 'inherit' });
}
