...
// Helper function to retry a task with a delay
async function retryTask(task, retries = 5, delay = 5000, errorMessage = 'Task failed') {
...
async function getCurrentProtocol(rpcPort, retries = 5, delay = 5000) {
    return retryTask(
        ...
        retries,
        delay,
        'Failed to retrieve current protocol'
    );
}
