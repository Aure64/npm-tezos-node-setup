...
async function waitForNodeToBootstrap(rpcPort) {
    ...
    // Initial delay to give the node time to start
    await new Promise(resolve => setTimeout(resolve, 10000)); // 10 seconds delay

    while (true) {
        try {
            const output = execSync(`octez-client --endpoint http://127.0.0.1:${rpcPort} bootstrapped`, { stdio: 'pipe' });
        ...
        await new Promise(resolve => setTimeout(resolve, 10000)); // Wait for another 10 seconds before checking again
    }
}
