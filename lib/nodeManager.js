...
// Function to retrieve the current Tezos protocol from the node
async function getCurrentProtocol(rpcPort) {
    try {
        const response = await axios.get(`http://127.0.0.1:${rpcPort}/chains/main/blocks/head`);  // Send a request to get the head block data
        return response.data.protocol;  // Return the protocol from the response data
    } catch (error) {
        console.error('Failed to retrieve current protocol:', error.message);  // Log any errors encountered
