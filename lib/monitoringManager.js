...
async function setupPyrometer(bakerAddress, alias, rpcPort) {
    console.log('Setting up Pyrometer for monitoring the baker...');

    // Check if Node.js is installed and is version 16 or later
    try {
...
async function postBakerSetup() {
    const { address: bakerAddress, alias } = getAddress();
    ...
    await setupPyrometer(bakerAddress, alias, chosenRpcPort);
}
